...
// place this file in the libraries file of your Arduino build.
...
#define SDA_PIN 50 // i2c data line
#define SCL_PIN 51 // i2c clock line
#define LED_PIN BOARD_LED_PIN // board led pin
#define SP1_NSS_PIN BOARD_SPI1_NSS_PIN
#define SP1_MOSI_PIN BOARD_SPI1_MOSI_PIN
#define SP1_MISO_PIN BOARD_SPI1_MISO_PIN
#define SP1_SCK_PIN BOARD_SPI1_SCK_PIN
#define T1_PIN 38
#define MOD0_PIN 7 //46
#define MOD1_PIN 47
...
void codec_maple_spi_setup(void) {
  ...
  pinMode(SP1_NSS_PIN, OUTPUT); // reset ss as output
}
...
void AudioCodec_init(void) {
  ...
  digitalWrite(SCL_PIN, HIGH); // release clock line
  digitalWrite(SDA_PIN, HIGH); // release data line
  pinMode(SCL_PIN, OUTPUT_OPEN_DRAIN); // setup clock line
  pinMode(SDA_PIN, OUTPUT_OPEN_DRAIN); // setup data line
  
  // initialize counter input pin
  pinMode(T1_PIN, INPUT); // set pin5 to input
  
  // initialize spi lines
  digitalWrite(SP1_MOSI_PIN, LOW); // set mosi low
  digitalWrite(SP1_SCK_PIN, LOW); // set sck low
  digitalWrite(SP1_NSS_PIN, LOW); // set ss low
  pinMode(SP1_SCK_PIN, OUTPUT); // set sck as output
  pinMode(SP1_NSS_PIN, OUTPUT); // set ss as output
  pinMode(SP1_MOSI_PIN, OUTPUT); // set mosi as output
  pinMode(SP1_MISO_PIN, INPUT); // set miso as input
  
  // initialize pot lines
  pinMode(MOD0_PIN, INPUT_ANALOG); // set mod0 as analog input
  pinMode(MOD1_PIN, INPUT_ANALOG); // set mod1 as analog input
